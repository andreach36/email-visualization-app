    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="flex flex-col px-6">
        <table class="table-fixed w-full border-collapse border border-gray-300">
          <thead>
            <tr class="bg-violet-300">
              <th class="border border-gray-300 px-4 py-2 text-xs font-medium text-gray-700">Subject</th>
              <th class="border border-gray-300 px-4 py-2 text-xs font-medium text-gray-700">From</th>
              <th class="border border-gray-300 px-4 py-2 text-xs font-medium text-gray-700">To</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="datarow in emailStore.dataEmails" :key="datarow.Id" class="bg-white">
              <td class="cursor-pointer border border-gray-300 px-4 py-2 text-center text-xs truncate text-ellipsis whitespace-nowrap" @click="showBody(datarow)">
                {{ datarow.Subject }}
              </td>
              <td class="cursor-pointer border border-gray-300 px-4 py-2 text-center text-xs truncate text-ellipsis whitespace-nowrap" @click="showBody(datarow)">
                {{ datarow.From }}
        ...
        <div class="flex justify-between items-center px-4 py-2 bg-gray-100 border-t">
          ...
          <span class="text-sm  text-gray-700">Page {{ emailStore.dataPagination.page_num }} from {{ emailStore.dataPagination.total_pages }}</span>
          <button class="px-2 py-1 bg-sky-800 text-white text-sm rounded disabled:opacity-50"
          ...
          <span class="text-sm  text-gray-700">Total Data: {{ emailStore.dataPagination.total_data }} emails</span>
        </div>
      </div>
      <!-- Detalles del email -->
      <div class="bg-white px-6 py-4 mr-8 rounded-lg h-[70vh] overflow-auto">
